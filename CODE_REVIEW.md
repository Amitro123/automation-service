...
The codebase generally adheres to the modular architecture described in `AGENTS.md` and `spec.md`. The separation of concerns between the webhook server, orchestrator, and task modules is well-implemented using `asyncio` for parallelism. However, there are critical bugs in the specification update logic, missing dependencies causing test failures, and discrepancies between the implementation and the requirements regarding output formats.
...
**Description:** The project uses `pytest` with `@pytest.mark.asyncio` decorators, but `pytest-asyncio` is not listed in `requirements.txt`.
**Impact:** Running `pytest` results in failures for all async tests (37 failures observed).
...
**Suggestion:** Use an async-native framework like Quart or FastAPI, or run the processing in a background thread/queue (though `asyncio.run` is "okay" for low volume).
...
*   `requirements.txt` has duplicate entries (`requests`, `python-dotenv`, etc.) and mixed version pinning.
